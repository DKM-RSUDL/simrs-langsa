...
class EdukasiAnestesiController extends Controller
{
    ...
    public function update($kd_pasien, $tgl_masuk, $urut_masuk, $idEncrypt, Request $request)
    {
        ...
        try {
            ...
            $edukasiAnestesi = OkEdukasiAnestesi::find($id);
            $oldpathFilePersetujuan = $edukasiAnestesi->file_persetujuan;

            $asesmen = OkAsesmen::find($edukasiAnestesi->id_asesmen);
            ...
            OkEdukasiAnestesi::where('id', $id)->update($data);

            // delete file lama
            if ($request->hasFile('file_persetujuan')) {
                if (!empty($oldpathFilePersetujuan)) {
                    if (Storage::exists($oldpathFilePersetujuan)) Storage::delete($oldpathFilePersetujuan);
                }
            }

            DB::commit();
            return to_route('operasi.pelayanan.asesmen.index', [$kd_pasien, $tgl_masuk, $urut_masuk])->with('success', 'Edukasi Anestesi berhasil di ubah !');
        } catch (Exception $e) {
            DB::rollBack();
            return back()->with('error', $e->getMessage());
        }
