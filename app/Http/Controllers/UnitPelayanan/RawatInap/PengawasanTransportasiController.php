...
class PengawasanTransportasiController extends Controller
{
    ...
    public function update($kd_unit, $kd_pasien, $tgl_masuk, $urut_masuk, $idEncrypt, Request $request)
    {
        ...
        try {
            ...
            $id = decrypt($idEncrypt);
            $pengawasan = RmePengawasanTransportasi::find($id);
            if (empty($pasien)) throw new Exception('Data pengawasan transportasi tidak ditemukan !');
            ...
            // delete dtl
            RmePengawasanTransportasiDtl::where('id_pengawasan', $pengawasan->id)->delete();
            ...
            DB::commit();
            return to_route('rawat-inap.pengawasan-transportasi.index', [$kd_unit, $kd_pasien, $tgl_masuk, $urut_masuk])->with('success', 'Pengawasan transportasi berhasil di ubah !');
        } catch (Exception $e) {
            DB::rollBack();
            return back()->with('error', $e->getMessage());
        }
    ...
    public function show($kd_unit, $kd_pasien, $tgl_masuk, $urut_masuk, $idEncrypt)
    {
        ...
        return view('unit-pelayanan.rawat-inap.pelayanan.pengawasan-transportasi.show', compact('dataMedis', 'dokter', 'perawat', 'petugas', 'pengawasan'));
    }
    ...
    public function delete($kd_unit, $kd_pasien, $tgl_masuk, $urut_masuk, Request $request)
    {
        ...
        try {
            $id = decrypt($request->id_pengawasan);
            $pengawasan = RmePengawasanTransportasi::find($id);

            if (empty($pengawasan)) throw new Exception('Data pengawasan transportasi tidak ditemukan !');

            RmePengawasanTransportasiDtl::where('id_pengawasan', $id)->delete();
            $pengawasan->delete();

            DB::commit();
            return back()->with('success', 'Data pengawasan transportasi berhasil dihapus !');
        } catch (Exception $e) {
            DB::rollBack();
            return back()->with('error', $e->getMessage());
        }
