        ...
        <div class="col-md-9">
            ...
            <div class="d-flex justify-content-between align-items-center m-3">
                <div class="row">
                    ...
                    <!-- Add Button -->
                    <div class="col-md-2">
                        <a href="{{ route('forensik.unit.pelayanan.pemeriksaan-patologi.create', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}"
                    ...
                    <tbody>
                        @foreach ($pemeriksaan as $item)
                            <tr>
                                ...
                                <td align="middle">
                                    <a href="{{ route('forensik.unit.pelayanan.pemeriksaan-patologi.show', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}" class="btn btn-sm btn-success"><i class="fas fa-eye"></i></a>
                                    <a href="{{ route('forensik.unit.pelayanan.pemeriksaan-patologi.edit', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}" class="btn btn-sm btn-warning mx-2"><i class="bi bi-pencil-square"></i></a>
        ...
        $('.btn-del-pemeriksaan').click(function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "post",
                        url: "{{ route('forensik.unit.pelayanan.pemeriksaan-patologi.destroy', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                            _method: "delete",
