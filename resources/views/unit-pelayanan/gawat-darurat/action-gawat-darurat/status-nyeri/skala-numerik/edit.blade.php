...
@section('content')
    <div class="row">
        ...
        <div class="col-md-9">
            <a href="{{ route('status-nyeri.skala-numerik.index', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}" class="btn btn-outline-primary mb-3">
                <i class="ti-arrow-left"></i> Kembali
            </a>

            <form id="painAssessmentForm" method="POST"
                action="{{ route('status-nyeri.skala-numerik.update', [$dataMedis->kd_pasien, $dataMedis->tgl_masuk, $dataMedis->urut_masuk, $skalaNumerik->id]) }}">
                @csrf
                @method('PUT')
