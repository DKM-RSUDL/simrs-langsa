        ...
        <div class="col-md-9">
            ...
            <x-content-card>
                @include('components.page-header', [
                    'title' => 'Daftar Persetujuan Transfusi Darah',
                    'description' => 'Daftar data persetujuan transfusi darah pasien gawat darurat.',
                ])
                            ...
                            <form method="GET" action="{{ request()->fullUrl() }}" class="d-flex">
                                ...
                                <div class="input-group">
                                    <input type="text" name="search" class="form-control"
                                        placeholder="Cari diagnosa, dokter, atau petugas" value="{{ request('search') }}">
                        ...
                        <tbody>
                            @forelse($persetujuanData as $index => $item)
                                <tr>
                                    ...
                                    <td>{{ $item->dokter ? $dokter->where('kd_dokter', $item->dokter)->first()->nama_lengkap ?? 'Tidak ada dokter dipilih' : 'Tidak ada dokter dipilih' }}
                                    </td>
                                    ...
                                    <td>
                                        <x-table-action>
                                            ...
                                            <!-- Delete Button -->
                                            <form
                                                action="{{ route('persetujuan-transfusi-darah.destroy', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}"
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <div>
                            <small class="text-muted">
                                Menampilkan {{ $persetujuanData->firstItem() ?? 0 }} -
                                {{ $persetujuanData->lastItem() ?? 0 }}
        ...
        $(document).ready(function() {
            ...
            $('.delete-form').on('submit', function(e) {
                ...
                if (typeof Swal !== 'undefined') {
                    ...
                    if (confirm(
                            'Apakah Anda yakin ingin menghapus data persetujuan transfusi darah ini?')) {
                        form.submit();
                    }
                }
            });
