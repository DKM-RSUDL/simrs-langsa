    ...
    <div class="row">
        <div>
            ...
            <div class="container-fluid">
                <form id="resepForm"
                    action="{{ route('farmasi.storeEResepPulang', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk))]) }}"
                    method="POST">
                    @csrf
                    <div class="row">
                        <!-- Side Column (Kiri) -->
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select select2" id="dokterPengirim"
                                                    ...
                                                    @foreach ($dokters as $dokter)
                                                        @if ($dokter && isset($dokter->kd_dokter))
                                    ...
                                    <table class="table table-bordered mt-3">
                                        ...
                                        <tbody>
                                            @forelse ($riwayatObat as $resep)
                                                ...
                                                @endphp
                                                <tr>
                                                    <td>{{ (int) ($resep->id_mrresep ?? 0) }}</td>
                                                    <td>Jenis Obat</td>
                                                    <td>{{ $resep->nama_obat ?? 'Tidak ada informasi' }}</td>
                                                    <td>{{ $resep->jumlah_takaran ?? '' }}
                                                        {{ Str::title($resep->satuan_takaran ?? '') }}</td>
                                                    <td>{{ $frekuensi }}</td>
                                                    <td>{{ (int) ($resep->jumlah ?? 0) ?: 'Tidak ada informasi' }}</td>
                                                    <td>Rute</td>
                                                    <td>{{ $keterangan }}</td>
                                                    <td>{{ $resep->ket ?? '' }}</td>
                                                    <td>{{ $resep->nama_dokter ?? 'Tidak ada informasi' }}</td>
                                                    <td>
                    ...
                    <div class="d-flex justify-content-end mt-3">
                        <a href="{{ route('farmasi.index', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk))]) }}"
        ...
        $(document).ready(function() {
            ...
            function submitResepForm(form) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        location.href =
                            "{{ route('farmasi.index', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk))]) }}";
                    },
                    error: function(xhr) {
                        $('#loadingIndicator').addClass('d-none');
                        $('#orderButton').prop('disabled', false);
            ...
            cariObat.on('input', function() {
                ...
                searchTimeout = setTimeout(() => {
                    ...
                    $.ajax({
                        url: "{{ route('farmasi.searchObat', [$dataMedis->kd_pasien, $dataMedis->tgl_masuk]) }}",
                        method: 'GET',
                        data: {
                            term: query
                        },
