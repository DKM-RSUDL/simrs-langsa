...
@section('content')
    <div class="row">
        ...
        <div class="col-md-9">
            <a href="{{ route('rawat-jalan.status-nyeri.skala-numerik.show', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $skalaNumerik->id]) }}" class="btn btn-outline-primary mb-3">
                <i class="ti-arrow-left"></i> Kembali
            </a>

            <form id="painAssessmentForm" method="POST"
                action="{{ route('rawat-jalan.status-nyeri.skala-numerik.update', [$dataMedis->kd_unit, $dataMedis->kd_pasien, $dataMedis->tgl_masuk, $dataMedis->urut_masuk, $skalaNumerik->id]) }}">
                @csrf
                @method('PUT')

                <div class="card shadow-sm">
                    <div class="card-body">
                        <h4 class="header-asesmen">Edit Pengkajian Status Nyeri Lanjutan Skala Numerik dan Wong Baker</h4>
...
@push('js')
    <script>
        $(document).ready(function() {
            ...
            // Initialize checkbox styling for checked items
        ...
        function initializeFormData() {
            // Initialize pain scale selection visual
            const selectedScale = $('input[name="pain_scale_type"]:checked').val();
            if (selectedScale) {
                $('.pain-scale-option[data-scale="' + selectedScale + '"]').addClass('selected');
            }

            // Initialize menjalar selection visual
            const menjalartValue = $('#menjalar_value').val();
            if (menjalartValue) {
            ...
            // Initialize radio item selection visual
            $('input[type="radio"]:checked').each(function() {
                $(this).closest('.radio-item').find('label').addClass('text-primary');
                $(this).closest('.radio-item').css({
                    'border-color': '#097dd6',
                    'background-color': '#e3f2fd'
                });
            });
