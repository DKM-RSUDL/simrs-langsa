...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-md-9">
            ...
            <form action="{{ route('rawat-jalan.tindakan-rehab-medik.store', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}" method="post">
                @csrf
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="ppa" id="ppa"
                                            ...
                                            @foreach ($petugas as $ptg)
                                                <option value="{{ $ptg->kd_karyawan }}" @selected($ptg->kd_karyawan == auth()->user()->kd_karyawan)>{{ $ptg->gelar_depan .' '. str()->title($ptg->nama) .' '. $ptg->gelar_belakang }}</option>
                                            @endforeach
                                        </select>
                                        ...
                                        <div class="row">
                                            <div class="col-7">
                                                <input type="date" name="tgl_tindakan" id="tgl_tindakan"
                                                    class="form-control @error('tgl_tindakan') is-invalid @enderror" value="{{ date('Y-m-d') }}" required>
                                            </div>
                                            <div class="col-5">
                                                <input type="time" name="jam_tindakan" id="jam_tindakan"
                                                    class="form-control @error('jam_tindakan') is-invalid @enderror" value="{{ date('H:i') }}" required>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-12">
                                        <div class="form-group">
                                            ...
                                            <textarea name="hasil" id="hasil" cols="30" rows="7"
                                                class="form-control @error('hasil') is-invalid @enderror" required></textarea>
                                        </div>
                                        @error('hasil')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                    ...
                                    <div class="col-12">
                                        <div class="form-group">
                                            ...
                                            <textarea name="rekomendasi" id="rekomendasi" cols="30" rows="5"
                                                class="form-control @error('rekomendasi') is-invalid @enderror" required></textarea>
                                        </div>
                                        @error('rekomendasi')
                                            <div class="invalid-feedback">
                                                {{ $message }}
