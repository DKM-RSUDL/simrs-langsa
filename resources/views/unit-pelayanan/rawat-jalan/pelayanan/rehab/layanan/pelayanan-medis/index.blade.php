<div>
    <div class="d-flex justify-content-start align-items-center m-3">
        <div class="row">
            ...
            <div class="col-md-2">
                <a href="{{ route('rawat-jalan.layanan-rehab-medik.create', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}" class="btn btn-primary">
            ...
            <tbody>
                @foreach ($layanan as $item)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('rawat-jalan.layanan-rehab-medik.show', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}" class="mb-2 btn btn-sm btn-success">
                                <i class="ti-eye"></i>
                            </a>
                            <a href="{{ route('rawat-jalan.layanan-rehab-medik.edit', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}" class="mb-2 btn btn-sm btn-warning">
                                <i class="ti-pencil"></i>
                            </a>
        ...
        $('.btn-delete-layanan').click(function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "post",
                        url: "{{ route('rawat-jalan.layanan-rehab-medik.destroy', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                            _method: "delete",
                            pelayanan: pelayanan
