...
@section('content')
    <div class="row">
        ...
        <div class="col-md-9">
            <a href="{{ route('rawat-jalan.audiometri.index', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}" class="btn btn-outline-primary mb-3">
                <i class="ti-arrow-left"></i> Kembali
            </a>

            <form id="audiometriForm" method="POST"
                action="{{ route('rawat-jalan.audiometri.update', [$dataMedis->kd_unit, $dataMedis->kd_pasien, $dataMedis->tgl_masuk, $dataMedis->urut_masuk, $dataAudiometri->id]) }}">
                @csrf
                @method('PUT')
