    ...
    <div class="row">
        <div>
            ...
            <div class="container-fluid">
                <form id="resepForm"
                    action="{{ route('rawat-inap.farmasi.storeEResepPulang', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}"
                    method="POST">
                    @csrf
                    <div class="row">
                        <!-- Side Column (Kiri) -->
            ...
            function submitResepForm(form) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        location.href =
                            "{{ route('rawat-inap.farmasi.index', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}";
                    },
                    error: function(xhr) {
                        $('#loadingIndicator').addClass('d-none');
                        $('#orderButton').prop('disabled', false);
