                    ...
                    <div class="card-body">
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active">
                                ...
                                <div class="row">
                                    <form method="GET" action="{{ route('rawat-inap.pneumonia.curb-65.index', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}">
                                        <div class="row m-3">
                                            <div class="col-md-3">
                                            ...
                                            <div class="col-md-3 text-end">
                                                <a href="{{ route('rawat-inap.pneumonia.curb-65.create', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}"
                                        ...
                                        <table class="table table-bordered table-sm table-hover">
                                            <thead class="table-primary">
                                                <tr>
                                                    ...
                                                    <th width="12%">Mortalitas</th>
                                                    <th width="15%">Level Resiko</th>
                                                    <th width="20%">Perawatan Disarankan</th>
                                                    <th width="13%">Petugas</th>
                                                    <th width="20%">Aksi</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @forelse($dataCurb65 as $index => $item)
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            <span class="badge bg-primary fs-6">{{ $item->total_skor }}</span>
                                                        </td>
                                                        <td class="text-center">{{ $item->mortalitas }}</td>
                                                        <td>
                                                            @php
                                                                $badgeClass = 'badge-resiko-rendah';
                                                                if(str_contains(strtolower($item->level_resiko), 'sedang-berat')) {
                                                                    $badgeClass = 'badge-resiko-sedang-berat';
                                                                } elseif(str_contains(strtolower($item->level_resiko), 'sedang')) {
                                                                    $badgeClass = 'badge-resiko-sedang';
                                                                } elseif(str_contains(strtolower($item->level_resiko), 'berat')) {
                                                                    $badgeClass = 'badge-resiko-berat';
                                                                }
                                                            @endphp
                                                            <span class="badge {{ $badgeClass }}">{{ $item->level_resiko }}</span>
                                                        </td>
                                                        <td>{{ $item->perawatan_disarankan }}</td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <div class="btn-group" role="group">
                                                                <a href="{{ route('rawat-inap.pneumonia.curb-65.show', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}"
                                                                    class="btn btn-sm btn-info" title="Lihat Detail">
                                                                    <i class="ti-eye"></i>
                                                                </a>
                                                                <a href="{{ route('rawat-inap.pneumonia.curb-65.edit', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}"
                                                            ...
                                                            <form id="delete-form-{{ $item->id }}" 
                                                                action="{{ route('rawat-inap.pneumonia.curb-65.destroy', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}" 
                                                    ...
                                                    <tr>
                                                        <td colspan="8" class="text-center py-4">
                                                            <div class="text-muted">
                                                                ...
                                                                <p class="mt-2">Belum ada data CURB-65</p>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                @endforelse
                                    ...
                                    @if($dataCurb65->isNotEmpty())
                                        <div class="mt-3">
                                            <small class="text-muted">
                                                Total: {{ $dataCurb65->count() }} data ditemukan
                                            </small>
                                        </div>
                                    @endif
                                </div>
