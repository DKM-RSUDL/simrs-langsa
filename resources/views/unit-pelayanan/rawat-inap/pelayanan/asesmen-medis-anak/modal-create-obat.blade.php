...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeObatData() {
        ...
        const rawData = riwayatObatDataInput.value;

        try {
            // Parse JSON dari input hidden
            riwayatObat = rawData ? JSON.parse(rawData) : [];
            // Pastikan riwayatObat adalah array
            if (!Array.isArray(riwayatObat)) {
                riwayatObat = [];
            }
        } catch (e) {
            console.error('Error parsing riwayatObatData:', e);
            riwayatObat = [];
        }
