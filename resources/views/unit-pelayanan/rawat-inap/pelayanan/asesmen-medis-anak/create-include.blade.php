...
@push('js')
    <script>
        ...
        // Pediatrik Forms
        document.addEventListener('DOMContentLoaded', function () {
            // Handle exclusive checkbox for Komplikasi
            const komplikasiTidak = document.getElementById('komplikasi_tidak');
            const komplikasiYa = document.getElementById('komplikasi_ya');

            if (komplikasiTidak && komplikasiYa) {
                komplikasiTidak.addEventListener('change', function () {
                    if (this.checked) {
                        komplikasiYa.checked = false;
                    }
                });

                komplikasiYa.addEventListener('change', function () {
                    if (this.checked) {
                        komplikasiTidak.checked = false;
                    }
                });
            }

            // Handle exclusive checkbox for Masalah Maternal
            const maternalTidak = document.getElementById('maternal_tidak');
            const maternalYa = document.getElementById('maternal_ya');
            const maternalKeterangan = document.querySelector('input[name="maternal_keterangan"]');

            if (maternalTidak && maternalYa && maternalKeterangan) {
                maternalTidak.addEventListener('change', function () {
                    if (this.checked) {
                        maternalYa.checked = false;
                        maternalKeterangan.disabled = true;
                        maternalKeterangan.value = '';
                    }
                });

                maternalYa.addEventListener('change', function () {
                    if (this.checked) {
                        maternalTidak.checked = false;
                        maternalKeterangan.disabled = false;
                        maternalKeterangan.focus();
                    } else {
                        maternalKeterangan.disabled = true;
                        maternalKeterangan.value = '';
                    }
            ...
            // Handle Lainnya sebutkan checkbox
            const lainnyaCheckbox = document.getElementById('lainnya_sebukan');
            const lainnyaTextarea = document.querySelector('textarea[name="lainnya_keterangan"]');

            if (lainnyaCheckbox && lainnyaTextarea) {
                lainnyaCheckbox.addEventListener('change', function () {
                    if (this.checked) {
                        lainnyaTextarea.disabled = false;
                        lainnyaTextarea.focus();
                    } else {
                        lainnyaTextarea.disabled = true;
                        lainnyaTextarea.value = '';
                    }
                });
            }

            // Handle exclusive checkbox for "Pernah dirawat sebelumnya"
            const pernahDirawatTidak = document.getElementById('pernah_dirawat_tidak');
            const pernahDirawatYa = document.getElementById('pernah_dirawat_ya');
            const tanggalDirawat = document.querySelector('input[name="tanggal_dirawat"]');
            const jamDirawat = document.querySelector('input[name="jam_dirawat"]');

            if (pernahDirawatTidak && pernahDirawatYa) {
                pernahDirawatTidak.addEventListener('change', function () {
                    if (this.checked) {
                        pernahDirawatYa.checked = false;
                        if (tanggalDirawat) {
                            tanggalDirawat.disabled = true;
                            tanggalDirawat.value = '';
                        }
                        if (jamDirawat) {
                            jamDirawat.disabled = true;
                            jamDirawat.value = '';
                        }
                    }
                });

                pernahDirawatYa.addEventListener('change', function () {
                    if (this.checked) {
                        pernahDirawatTidak.checked = false;
                        if (tanggalDirawat) {
                            tanggalDirawat.disabled = false;
                            tanggalDirawat.focus();
                        }
                        if (jamDirawat) {
                            jamDirawat.disabled = false;
                        }
                    } else {
                        if (tanggalDirawat) {
                            tanggalDirawat.disabled = true;
        ...
        function resetDischargePlanning() {
            const elements = [
                'select[name="usia_lanjut"]',
                'select[name="hambatan_mobilisasi"]',
                'select[name="penggunaan_media_berkelanjutan"]',
                'select[name="ketergantungan_aktivitas"]'
            ];
            ...
            const alerts = ['.alert-info', '.alert-warning', '.alert-success'];
        ...
        function showMilestoneGuide() {
            const guide = document.getElementById('milestoneGuide');
            if (guide) {
        ...
        function clearForm() {
            if (confirm('Apakah Anda yakin ingin menghapus semua data yang telah diisi?')) {
                ...
                // Reset JSON hidden inputs
                const jsonInputs = ['riwayatObatData', 'alergisInput', 'diagnosis_banding', 'diagnosis_kerja'];
                jsonInputs.forEach(inputId => {
                    const input = document.getElementById(inputId);
                    if (input) input.value = '[]';
                });
                ...
                if (typeof obatManagement !== 'undefined') {
                    obatManagement.data = [];
                    obatManagement.renderTable();
                }

                if (typeof alergiManagement !== 'undefined') {
                    alergiManagement.data = [];
                    alergiManagement.renderTable();
                }
        ...
        console.log('Enhanced JavaScript with JSON formatting loaded successfully');
    </script>
@endpush
