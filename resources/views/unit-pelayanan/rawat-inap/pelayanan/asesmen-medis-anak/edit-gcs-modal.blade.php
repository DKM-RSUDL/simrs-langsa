...
function loadExistingGCSData() {
    ...
    if (gcsInput && gcsInput.value && gcsInput.value.trim()) {
        ...
        // Reset modal jika tidak ada data
        resetGCSModal();
    }
}

/**
 * Reset semua radio button dan display di modal
 */
...
/**
 * Parse format GCS "6 E2 V2 M2" dan set ke radio buttons
 */
function parseAndSetGCSData(gcsString) {
    try {
        // Pastikan gcsString adalah string yang valid
        if (!gcsString || typeof gcsString !== 'string') {
            throw new Error('Invalid GCS string');
        }

        // Parse format: "6 E2 V2 M2"
        ...
        // Validasi nilai
        if (eyeValue && !isNaN(eyeValue) && parseInt(eyeValue) >= 1 && parseInt(eyeValue) <= 4) {
        ...
        if (verbalValue && !isNaN(verbalValue) && parseInt(verbalValue) >= 1 && parseInt(verbalValue) <= 5) {
        ...
        if (motoricValue && !isNaN(motoricValue) && parseInt(motoricValue) >= 1 && parseInt(motoricValue) <= 6) {
