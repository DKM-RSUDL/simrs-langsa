...
@push('js')
        ...
        $(document).ready(function() {
            $('#SelectOption').change(function() {
                ...
                window.location.href =
                    "{{ route('rawat-inap.tindakan.index', ['kd_unit' => $dataMedis->kd_unit, 'kd_pasien' => $dataMedis->kd_pasien, 'tgl_masuk' => \Carbon\Carbon::parse($dataMedis->tgl_masuk)->format('Y-m-d'), 'urut_masuk' => $dataMedis->urut_masuk]) }}" +
                    queryString;
            });
        });
    ...
    <script>
        ...
        $('.btn-edit-tindakan').click(function(e) {
            ...
            $.ajax({
                type: "post",
                url: "{{ route('rawat-inap.tindakan.get-tindakan-ajax', [$dataMedis->kd_unit, $dataMedis->kd_pasien, $dataMedis->tgl_masuk, $dataMedis->urut_masuk]) }}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    "kd_produk": kdProduk,
                    "urut_list": urut,
        ...
        $('.btn-delete-tindakan').click(function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "post",
                        url: "{{ route('rawat-inap.tindakan.delete', [$dataMedis->kd_unit, $dataMedis->kd_pasien, $dataMedis->tgl_masuk, $dataMedis->urut_masuk]) }}",
                        data: {
                            '_method': 'delete',
                            '_token': "{{ csrf_token() }}",
                            "kd_produk": kdProduk,
