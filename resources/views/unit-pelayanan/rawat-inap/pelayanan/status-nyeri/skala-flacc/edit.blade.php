...
@section('content')
    <div class="row">
        ...
        <div class="col-md-9">
            <a href="{{ route('rawat-inap.status-nyeri.skala-flacc.show', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $skalaFlacc->id]) }}" class="btn btn-outline-primary mb-3">
                <i class="ti-arrow-left"></i> Kembali
            </a>

            <form id="flaccAssessmentForm" method="POST"
                action="{{ route('rawat-inap.status-nyeri.skala-flacc.update', [$dataMedis->kd_unit, $dataMedis->kd_pasien, $dataMedis->tgl_masuk, $dataMedis->urut_masuk, $skalaFlacc->id]) }}">
                @csrf
                @method('PUT')
