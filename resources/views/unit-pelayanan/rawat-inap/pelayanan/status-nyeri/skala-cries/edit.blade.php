...
@section('content')
    <div class="row">
        ...
        <div class="col-md-9">
            <a href="{{ route('rawat-inap.status-nyeri.skala-cries.show', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $skalaCries->id]) }}" class="btn btn-outline-primary mb-3">
                <i class="ti-arrow-left"></i> Kembali
            </a>

            <form id="criesAssessmentForm" method="POST"
                action="{{ route('rawat-inap.status-nyeri.skala-cries.update', [$dataMedis->kd_unit, $dataMedis->kd_pasien, $dataMedis->tgl_masuk, $dataMedis->urut_masuk, $skalaCries->id]) }}">
                @csrf
                @method('PUT')
