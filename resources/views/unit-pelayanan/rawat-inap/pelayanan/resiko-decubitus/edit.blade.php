...
@section('content')
    <div class="row">
        ...
        <div class="col-md-9">
            <a href="{{ route('rawat-inap.resiko-decubitus.index', [$dataMedis->kd_unit, $dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}" class="btn btn-outline-primary mb-3">
                <i class="ti-arrow-left"></i> Kembali
            </a>

            <form id="nortonAssessmentForm" method="POST"
                action="{{ route('rawat-inap.resiko-decubitus.update', [$dataMedis->kd_unit, $dataMedis->kd_pasien, $dataMedis->tgl_masuk, $dataMedis->urut_masuk, $dataDecubitus->id]) }}">
                @csrf
                @method('PUT')
    ...
    <script>
        ...
        function checkDuplicateDateTimeEdit() {
            ...
            if (tanggal && shift && hariKe) {
                ...
                $.ajax({
                    url: "{{ route('rawat-inap.resiko-decubitus.check-duplicate', [$dataMedis->kd_unit, $dataMedis->kd_pasien, $dataMedis->tgl_masuk, $dataMedis->urut_masuk]) }}",
                    method: 'POST',
                    data: {
