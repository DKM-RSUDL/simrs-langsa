...
@include('unit-pelayanan.rawat-inap.pelayanan.persetujuan-transfusi-darah.include')
...
<div class="row">
    ...
    <div class="col-md-9">
        ...
        <x-content-card>
            ...
            'description' =>
            'Tambah data persetujuan transfusi darah pasien hemodialisa di unit pelayanan hemodialisa.',
            ])
            ...
            <div class="row mb-3">
                <div class="col-md-12">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <!-- Add Button -->
                        <div>
                            <a href="{{ route('hemodialisa.pelayanan.persetujuan-transfusi-darah.create', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}"
                ...
                <table class="table table-bordered table-sm table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item->dokter ? $dokter->where('kd_dokter', $item->dokter)->first()->nama_lengkap ??
                                'Tidak ada dokter dipilih' : 'Tidak ada dokter dipilih' }}
                            </td>
                            <td>{{ str()->title($item->userCreate->name ?? 'Tidak Diketahui') }}</td>
                            <td>
                                <div class="btn-group" role="group">
                                    <!-- Detail Button -->
                                    <a href="{{ route('hemodialisa.pelayanan.persetujuan-transfusi-darah.show', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}"
                                        class="btn btn-info btn-sm" title="Detail">
                                        <i class="ti-eye"></i>
                                    </a>

                                    <!-- Edit Button -->
                                    <a href="{{ route('hemodialisa.pelayanan.persetujuan-transfusi-darah.edit', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}"
                                    ...
                                    <!-- Delete Button -->
                                    <form
                                        action="{{ route('hemodialisa.pelayanan.persetujuan-transfusi-darah.destroy', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}"
