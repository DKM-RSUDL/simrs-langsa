                    ...
                    <div class="card-body">
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active">
                                ...
                                <div class="row">
                                    <form method="GET"
                                        action="{{ route('hemodialisa.pelayanan.persetujuan.akses-femoralis.index', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}">
                                        <div class="row m-3">
                                            <div class="col-md-3">
                                                ...
                                                @forelse ($dataPersetujuan as $key => $item)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <small
                                                                class="text-muted">{{ date('H:i', strtotime($item->jam_implementasi)) }}
                                                                WIB</small>
                                                        </td>
                                                        <td>
                                                            @if ($item->dokter)
                                                        ...
                                                        <td>
                                                            ...
                                                            @if ($item->tipe_penerima == 'keluarga' && $item->nama_keluarga)
                                                                ...
                                                                @if ($item->status_keluarga)
                                                                    <br><small
                                                                        class="text-muted">({{ ucfirst(str_replace('_', ' ', $item->status_keluarga)) }})</small>
                                                                @endif
                                                            @endif
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            @php
                                                                $infoCount = 0;
                                                                if ($item->info_diagnosis) {
                                                                    $infoCount++;
                                                                }
                                                                if ($item->info_dasar_diagnosis) {
                                                                    $infoCount++;
                                                                }
                                                                if ($item->info_tindakan) {
                                                                    $infoCount++;
                                                                }
                                                                if ($item->info_indikasi) {
                                                                    $infoCount++;
                                                                }
                                                                if ($item->info_tata_cara) {
                                                                    $infoCount++;
                                                                }
                                                                if ($item->info_tujuan) {
                                                                    $infoCount++;
                                                                ...
                                                                if ($item->info_prognosis) {
                                                                    $infoCount++;
                                                                }
                                                                if ($item->info_alternatif) {
                                                                    $infoCount++;
                                                                }
                                                                if ($item->info_lain_lain_check) {
                                                                    $infoCount++;
                                                                }
                                                            @endphp
                                                        ...
                                                        <td>
                                                            <div class="btn-group" role="group">
                                                                ...
                                                                <form
                                                                    action="{{ route('hemodialisa.pelayanan.persetujuan.akses-femoralis.destroy', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}"
                                                    ...
                                                    <tr>
                                                        <td colspan="8" class="text-center">
                                                            <div class="py-3">
                                                                ...
                                                                <p class="mt-2 text-muted">Belum ada data persetujuan
                                                                    tindakan akses femoralis</p>
                                                                <a href="{{ route('hemodialisa.pelayanan.persetujuan.akses-femoralis.create', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk]) }}"
