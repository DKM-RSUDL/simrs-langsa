        ...
        <div class="col-md-9">
            ...
            <x-content-card>
                @include('components.page-header', [
                    'title' => 'Daftar Data Traveling Dialysis',
                    'description' => 'Daftar data traveling dialysis pada hemodialisa.',
                ])
                <!-- Header -->
                <div class="text-end">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if ($item->time_first_dialysis)
                                            <br>
                                            <small
                                                class="text-muted">{{ \Carbon\Carbon::parse($item->time_first_dialysis)->format('H:i') }}</small>
                                        @endif
                                    </td>
                                            ...
                                            @if ($item->hbsag_result || $item->anti_hcv_result || $item->anti_hiv_result)
                                                @if ($item->hbsag_result)
                                                    <div class="test-item mb-1">
                                                        <span
                                                            class="badge {{ $item->hbsag_result == 'Reactive (+)' ? 'badge-danger' : 'badge-success' }} me-1">
                                                            HbsAg: {{ $item->hbsag_result }}
                                                        </span>
                                                        @if ($item->hbsag_date)
                                                            <small
                                                                class="text-muted d-block">{{ \Carbon\Carbon::parse($item->hbsag_date)->format('d/m/Y') }}</small>
                                                        @endif
                                                    </div>
                                                @endif
                                                ...
                                                @if ($item->anti_hcv_result)
                                                    <div class="test-item mb-1">
                                                        <span
                                                            class="badge {{ $item->anti_hcv_result == 'Reactive (+)' ? 'badge-danger' : 'badge-success' }} me-1">
                                                            Anti HCV: {{ $item->anti_hcv_result }}
                                                        </span>
                                                        @if ($item->anti_hcv_date)
                                                            <small
                                                                class="text-muted d-block">{{ \Carbon\Carbon::parse($item->anti_hcv_date)->format('d/m/Y') }}</small>
                                                        @endif
                                                    </div>
                                                @endif
                                                ...
                                                @if ($item->anti_hiv_result)
                                                    <div class="test-item mb-1">
                                                        <span
                                                            class="badge {{ $item->anti_hiv_result == 'Reactive (+)' ? 'badge-danger' : 'badge-success' }} me-1">
                                                            Anti HIV: {{ $item->anti_hiv_result }}
                                                        </span>
                                                        @if ($item->anti_hiv_date)
                                                            <small
                                                                class="text-muted d-block">{{ \Carbon\Carbon::parse($item->anti_hiv_date)->format('d/m/Y') }}</small>
                                                        @endif
                                                    </div>
                                                @endif
                                            @else
                                    ...
                                    <td>
                                        ...
                                        <div class="d-flex flex-wrap gap-1 mt-1">
                                            ...
                                            <form
                                                action="{{ route('hemodialisa.pelayanan.traveling-dialysis.destroy', [$dataMedis->kd_pasien, date('Y-m-d', strtotime($dataMedis->tgl_masuk)), $dataMedis->urut_masuk, $item->id]) }}"
